...
- [x] **Sync settings + native git bridge handler.** A Settings entry for
...
- [x] **Write-on-save → background commit + push.** On collection/env
...
- [x] **Pull-on-launch.** When sync is configured, `git pull` at startup
  before hydrating the store. Touchpoints: app init, git handler.
- [x] **Conflict resolution.** On a pull conflict, surface a manual diff;
