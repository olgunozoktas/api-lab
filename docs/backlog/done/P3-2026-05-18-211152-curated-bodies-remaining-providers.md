...
- [x] GitHub — JSON body skeletons for Create repo, Create issue,
      Update issue, Create pull request; descriptions on all.
- [x] OpenAI — JSON skeletons for Create chat completion, Create
      embeddings, Create image, Create moderation, Create batch (the
      common ones); descriptions on all.
- [x] Slack — JSON skeletons for Post message, Update message, Create
      channel, Add reaction; descriptions on all.
- [x] Notion — JSON skeletons for Create page, Query database, Search,
      Append block children; descriptions on all. (Note: Notion needs
      a `Notion-Version` header — already mentioned in its file
      header.)
- [x] Linear — seed the single GraphQL endpoint with a starter query
