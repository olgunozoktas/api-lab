...
export function HistoryList({ query = "" }: { query?: string }) {
  ...
  const filtered = useMemo(
    () => (filtering ? filterHistory(history, trimmedQuery) : history),
    [history, filtering, trimmedQuery]
  );
      ...
      {filtered.slice(0, 100).map((h) => {
        ...
        return (
          ...
          >
            <span
              className={
                "font-mono font-bold w-9 flex-shrink-0 text-[10px] " + methodClass(h.request.method)
              }
            >
              {h.request.method}
            </span>
