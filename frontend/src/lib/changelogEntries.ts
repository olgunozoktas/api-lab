...
// Changelog content layer — the build-time glob of every changelog
// markdown file plus the locale picker over it. Split out of
// `lib/changelog.ts` so the light version helpers (APP_VERSION,
// cmpVersion …) can be imported eagerly by the TopBar / Settings /
// update-check / changelog-gate without dragging the whole changelog
// prose corpus into the first-paint chunk. This module is imported
// only by the lazy-loaded <ChangelogModal>, so Rolldown keeps the
// glob content in that modal's async chunk.
...
//
// Path shape (relative to this file at frontend/src/lib/changelogEntries.ts):
...
// New callers should use selectChangelogEntries(CHANGELOG_ENTRIES, locale).
